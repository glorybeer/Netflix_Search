...
def fetch_kmrb_rating(title):
    """영등위(KMRB) Open API 조회 - 올바른 엔드포인트 사용"""
    cleaned = clean_title_text(title)
    if not cleaned:
        cleaned = title

    # 정확한 KMRB 엔드포인트
    url = "http://apis.data.go.kr/B551008/irating_v1/ir_search"
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if items:
                ...
                scores = {
                    'theme': parse_val(item.get('thme')),
                    'sensuality': parse_val(item.get('sensxlty')),
                    'violence': parse_val(item.get('vlet')),
                    'dialogue': parse_val(item.get('dlog')),
                    'horror': parse_val(item.get('hror')),
                    'drug': parse_val(item.get('drg')),
                    'imitation': parse_val(item.get('imit'))
                }
                return scores, True
            else:
